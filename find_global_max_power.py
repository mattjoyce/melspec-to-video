...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", help="Path to the configuration YAML file", required=True
    )
    parser.add_argument(
        "--audio", help="Path to the audio file to process", required=True
    )
    ...
    parser.add_argument(
        "--sr",
        help="Sample rate to override librosa's default",
        type=int,
        default=22050,
    )
    parser.add_argument(
    ...
    if args.chunk_duration:
        ...
        process_audio(
            args.config, args.audio, args.start, args.duration, args.sr, float("inf")
        )  # Treat as a single chunk if no chunk_duration provided
